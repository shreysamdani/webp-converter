...
const anim_dump = require("./anim_dump.js");
...
module.exports.str2webpstr = (base64str, image_type, option) => {
  ...
  return buffer_utils
    .base64str2webp(base64str, image_type, option)
    .then(function (val) {
      return val;
    });
};
...
module.exports.buffer2webpbuffer = (buffer, image_type, option) => {
  ...
  //option: options and quality,it should be given between 0 to 100
  return buffer_utils
    .buffer2webp(buffer, image_type, option)
    .then(function (val) {
      return val;
    });
};
...
module.exports.anim_dump = (
  ...
  // input_image: input image(.webp)
  // start_frame: 0 indexed, inclusive - first frame in range to dump
  // end_frame: 0 indexed, exclusive - last frame in range to dump
  // output_folder: folder to dump frame files in
  // output files will look like xxxxxxx.png

  const query = `-start_frame ${start_frame} -end_frame ${end_frame} -folder "${output_folder}" "${input_image}" `;

  //anim_dump() return which platform webp library should be used for conversion
  return new Promise((resolve, reject) => {
    //execute command
    exec(
      `"${anim_dump()}"`,
